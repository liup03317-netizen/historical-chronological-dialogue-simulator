...
| 优先级 | 数据源 | 搜索键 | CORS 支持 | 浏览器直连 | 说明 |
|--------|--------|--------|-----------|-----------|------|
| 1 | 中文维基百科 | 英文 WikiTitle（自动重定向至中文） | ✅ `origin=*` | ✅ | 首选，跨语种重定向优雅 |
| 1.5 | 英文维基百科 | 英文 WikiTitle | ✅ `origin=*` | ✅ | 中文维基无结果时兜底 |
...
当中文维基百科返回无结果（`pageId` 为 `-1`）时，使用相同的 WikiTitle 请求英文维基百科进行兜底。这确保那些拥有英文维基百科页面但无中文维基百科页面的人物仍能获取肖像头像。

### API 端点

```
https://en.wikipedia.org/w/api.php?action=query&titles={WikiTitle}&prop=pageimages|extracts&exsentences=2&exintro=true&explaintext=true&format=json&pithumbsize=200&origin=*
```
...
| 差异点 | 中文维基 | 英文维基兜底 |
|--------|----------|-------------|
| 域名 | `zh.wikipedia.org` | `en.wikipedia.org` |
| `redirects` 参数 | `redirects=1`（跨语种重定向） | 不需要（直接匹配英文标题） |
| 简介语言 | 中文 | 英文 |
| 使用场景 | 首选 | 中文维基无结果时兜底 |

### 说明

- 英文维基兜底时返回的简介（`extract`）为英文，这是可接受的，因为肖像头像是主要目标
- 英文维基同样支持 CORS（`origin=*`），在纯浏览器环境下可直接使用
- WikiTitle 本身即为英文，因此无需 `redirects=1` 参数即可直接匹配
